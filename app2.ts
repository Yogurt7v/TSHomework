...
const normalizeData = (unnormalizedData: Post[]): { byId: ByIdPosts, allIds: string[] } => {
  let byId: ByIdPosts = {};
  for (const item of unnormalizedData) {
    const { id, title, body } = item;
    byId[id] = { id, title, body };
  }
  let allIds: string[] = [];
  unnormalizedData.map((item) => allIds!.push(item.id));
